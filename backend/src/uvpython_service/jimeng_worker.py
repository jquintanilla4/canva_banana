...
def _resolve_jimeng_executable(settings: Settings) -> str:
    ...
    if explicit_path:
        ...
        resolved_explicit = shutil.which(explicit_path)
    ...
    local_bin_dir = settings.jimeng_work_dir.parent / ".tools" / "jimeng" / "bin"
    for local_name in ("dreamina", "jm"):
        local_candidate = local_bin_dir / local_name
        if local_candidate.exists():
            return str(local_candidate)  # Prefer project-local tools when present.

    for executable_name in ("dreamina", "jm"):
        resolved = shutil.which(executable_name)
        if resolved:
            return resolved  # Fall back to global PATH installs from the official script.

    raise RuntimeError(
        f"Jimeng CLI not found. Install or update with `{JIMENG_INSTALL_COMMAND}`, "
        "or set JIMENG_CLI_PATH to the dreamina executable."
    )
