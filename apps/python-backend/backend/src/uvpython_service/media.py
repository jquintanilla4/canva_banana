...
def get_ffprobe_status() -> FfprobeStatus:
    ...
    return FfprobeStatus(
        ...
        path=None,
        source="missing",
        warning=invalid_explicit_warning or "ffprobe is unavailable; rerun `npm -w @canva-banana/python-backend run sync`, set `VOLCENGINE_FFPROBE_PATH`, or install ffmpeg system-wide.",
    )
