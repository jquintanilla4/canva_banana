...
const VOLCENGINE_LOCAL_ACTION_HEADER = 'X-Canva-Banana-Local-Action'; // Backend checks this marker on generation submits.
...
const VOLCENGINE_BACKEND_UNREACHABLE_MESSAGE_SUFFIX = 'Run `npm -w @canva-banana/python-backend run sync` once, then `npm run backend:dev`.'; // Point local setup failures at the expected commands.
...
const getVolcengineApiBaseUrl = (): string => {
  const raw = getRuntimeConfig().volcengineApiBaseUrl?.trim() || DEFAULT_VOLCENGINE_API_BASE_URL;
  return raw.replace(/\/+$/, ''); // Avoid double slashes when building paths.
};

const getVolcengineActionHeaders = (baseUrl: string): Record<string, string> => {
  return {
    [VOLCENGINE_LOCAL_ACTION_HEADER]: 'volcengine-submit',
...
export const generateSeedanceVideo = async (
  ...
  const submitResponse = await fetchVolcengine(baseUrl, '/api/volcengine/jobs', 'submit', {
    method: 'POST',
    headers: getVolcengineActionHeaders(baseUrl),
    body: formData,
  });
