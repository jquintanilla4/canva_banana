...
const getBackendUnreachableMessage = (baseUrl = DEFAULT_BASE_URL): string =>
  `Seedance 2 could not reach the local Volcengine backend at ${baseUrl}. Run \`npm -w @canva-banana/python-backend run sync\` once, then \`npm run backend:dev\`.`; // Mirror the service message exactly so regressions fail loudly.
...
describe('volcengineService', () => {
  ...
  it('does not send the desktop token to an env-overridden Volcengine backend origin', async () => {
    const remoteBaseUrl = 'https://remote-volcengine.example.test/backend';
    window.canvaBananaDesktop = {
      getRuntimeConfig: () => ({
        isDesktop: true,
        pythonBackendAuthToken: 'desktop-secret',
        pythonBackendAuthOrigin: DEFAULT_BASE_URL,
        volcengineApiBaseUrl: remoteBaseUrl,
      }),
    };
    global.fetch = vi.fn()
      .mockResolvedValueOnce(createHealthResponse())
    ...
    await expect(generateSeedanceVideo('test prompt', createOptions())).resolves.toEqual({
      videoUrl: 'https://example.com/output.mp4',
      providerVideoUrl: undefined,
      requestId: undefined,
      lastFrameUrl: undefined,
      providerLastFrameUrl: undefined,
    });
    expect(global.fetch).toHaveBeenNthCalledWith(2, `${remoteBaseUrl}/api/volcengine/jobs`, expect.objectContaining({
